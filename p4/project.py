...
@app.route('/catalog/<int:cat_id>/menu/JSON')
...
@app.route('/catalog/menu/JSON')
def categoryMenuJSONAll():
    items = session.query(MenuItem).all()
    return jsonify(MenuItems=[i.serialize for i in items])


@app.route('/catalog/<int:cat_id>/menu/<int:menu_id>/JSON')
def menuItemJSON(cat_id, menu_id):
...
def showCategories():
    ...
    items = session.query(MenuItem).order_by(desc(MenuItem.id)).limit(8).all()
...
def showItems(cat_name):
    ...
    items = session.query(MenuItem).filter_by(cat_id=category.id).all()
...
def newItem():
    ...
    if request.method == 'POST':
        newItem = MenuItem(name=request.form['name'], description=request.form['description'], cat_id=request.form['category'], user_id=login_session['user_id'])
...
# Edit a menu item
@app.route('/catalog/<int:cat_id>/menu/<int:menu_id>/edit', methods=['GET', 'POST'])
def editItem(cat_id, menu_id):
    if 'username' not in login_session:
        return redirect('/login')
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('showItems', cat_name=category.name))
    else:
        categories = session.query(Category).order_by(asc(Category.id))
        return render_template('edititem.html', cat_id=cat_id, menu_id=menu_id, item=editedItem, categories=categories)


# Delete a menu item
@app.route('/catalog/<int:cat_id>/menu/<int:menu_id>/delete', methods=['GET', 'POST'])
def deleteItem(cat_id, menu_id):
    if 'username' not in login_session:
        return redirect('/login')
    category = session.query(Category).filter_by(id=cat_id).one()
